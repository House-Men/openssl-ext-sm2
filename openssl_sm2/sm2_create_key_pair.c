...
int generate_pubic_key(SM2_KEY_PAIR *key_pair) {

    int error_code;
    //初始化了一个EC_KEY结构来存储一对密钥对
    EC_KEY *key = NULL;
    //初始化私钥、公钥X、公钥Y容器
    ...
    EC_POINT *ec_pt = EC_POINT_new(group);
    EC_POINT_mul(group, ec_pt, bn_d, NULL, NULL, ctx);
    EC_KEY_set_public_key(key, ec_pt);
    error_code = CREATE_SM2_KEY_PAIR_FAIL;
    if (!(EC_POINT_get_affine_coordinates_GFp(group,
                                              ec_pt,
                                              bn_x,
